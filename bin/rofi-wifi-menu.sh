...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | rofi -i -dmenu -p "Wi-Fi SSID: " -lines "$LINENUM" -a "$HIGHLINE" -location "$POSITION" -yoffset "$YOFF" -xoffset "$XOFF" -width -"$RWIDTH")
#echo "$CHENTRY"
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") != "" ]]; then
		nmcli con up "$CHSSID"
	else
